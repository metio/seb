...
The special syntax `::set-output name=<NAME>::` declares that the output of the command (`echo`) should save its output to a variable called `<NAME>`. Together with the `<ID>` of the pipeline step, this value can be referenced like `${{ steps.<ID>.outputs.<NAME> }}` in the next steps of your pipeline.
